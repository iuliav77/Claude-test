...
def simulation_loop():
    ...
    print("🔄 v1.0 Simulation loop started (drift-safe, 5-minute intervals)")
    ...
    while _simulator and _shutdown_event and not _shutdown_event.is_set():
        ...
        if time_until_tick < 0:
            ...
            if missed_seconds > 10:  # More than 10 seconds late
                print(f"⚠ Missed tick by {missed_seconds:.1f}s - adjusting schedule")
        ...
        try:
            ...
            if current_state:
                ...
                print(f"✓ Tick: {current_state.timestamp.strftime('%H:%M:%S')} | "
...
def stop_simulation_loop():
    ...
    if _shutdown_event:
        print("🛑 Requesting simulation loop shutdown...")
        _shutdown_event.set()
    
    if _simulation_thread and _simulation_thread.is_alive():
        _simulation_thread.join(timeout=5)
        if _simulation_thread.is_alive():
            print("⚠ Simulation thread did not stop in time")
        else:
            print("✓ Simulation thread stopped cleanly")
...
def state_to_response(state: TurbineState, simulator: Simulator) -> CurrentStateResponse:
    ...
    # v1.0: Bulletproof status conversion (handles int, enum, or string)
    def safe_status_string(status) -> str:
        """Convert any status representation to string safely."""
        try:
            if isinstance(status, str):
                return status
            elif isinstance(status, int):
                # Map integer to enum name
                status_map = {
                    0: "STOPPED",
                    1: "STARTING", 
                    2: "RUNNING",
                ...
                return str(status.value) if isinstance(status.value, str) else status.name
    ...
    status_str = safe_status_string(state.status)
    ...
    return CurrentStateResponse(
        ...
        generator_temp_c=clamp(state.generator_temp_c, -50.0, 200.0),  # Max 200°C
...
def get_maintenance_recommendations(state: TurbineState) -> List[MaintenanceRecommendation]:
    """
    Generate maintenance recommendations based on state (v1.0: enhanced trending).
    
    v1.0 REALISM FIX: Add warnings before critical thresholds to show degradation.
    """
    recommendations = []
    
    # Temperature checks - multiple warning levels
    if state.generator_temp_c > 110:
        recommendations.append(MaintenanceRecommendation(
            component="Generator",
            priority="critical",
            message="Generator temperature CRITICAL - shutdown recommended",
            reason=f"Temperature: {state.generator_temp_c:.1f}°C (critical limit: 110°C)"
        ))
    elif state.generator_temp_c > 100:
        recommendations.append(MaintenanceRecommendation(
            component="Generator",
            priority="high",
            message="Generator temperature exceeds safe operating limit",
            reason=f"Temperature: {state.generator_temp_c:.1f}°C (limit: 100°C)"
        ))
        ...
        recommendations.append(MaintenanceRecommendation(
            ...
            reason=f"Temperature: {state.generator_temp_c:.1f}°C (warning threshold: 90°C)"
        ))
    elif state.generator_temp_c > 75:
        recommendations.append(MaintenanceRecommendation(
            component="Generator",
            priority="low",
            message="Generator temperature trending high",
            reason=f"Temperature: {state.generator_temp_c:.1f}°C (monitor above 75°C)"
        ))
    
    if state.gearbox_temp_c > 100:
        recommendations.append(MaintenanceRecommendation(
            component="Gearbox",
            priority="critical",
            message="Gearbox temperature CRITICAL",
            reason=f"Temperature: {state.gearbox_temp_c:.1f}°C (critical: 100°C)"
        ))
    elif state.gearbox_temp_c > 90:
        recommendations.append(MaintenanceRecommendation(
            component="Gearbox",
            priority="high",
            message="Gearbox temperature exceeds safe operating limit",
            reason=f"Temperature: {state.gearbox_temp_c:.1f}°C (limit: 90°C)"
        ))
        ...
        recommendations.append(MaintenanceRecommendation(
            ...
            reason=f"Temperature: {state.gearbox_temp_c:.1f}°C (warning: 80°C)"
        ))
    elif state.gearbox_temp_c > 70:
        recommendations.append(MaintenanceRecommendation(
            component="Gearbox",
            priority="low",
            message="Gearbox temperature increasing",
            reason=f"Temperature: {state.gearbox_temp_c:.1f}°C (monitor above 70°C)"
        ))
    
    # Vibration checks - multiple levels
    if state.vibration_mms > 8:
        recommendations.append(MaintenanceRecommendation(
            component="Bearings",
            priority="critical",
            message="CRITICAL vibration - immediate inspection required",
            reason=f"Vibration: {state.vibration_mms:.2f} mm/s (critical: 8 mm/s)"
        ))
    elif state.vibration_mms > 7:
        recommendations.append(MaintenanceRecommendation(
            component="Bearings",
            priority="high",
            message="Excessive vibration detected",
            reason=f"Vibration: {state.vibration_mms:.2f} mm/s (high threshold: 7 mm/s)"
        ))
    ...
    elif state.vibration_mms > 3.5:
        recommendations.append(MaintenanceRecommendation(
            component="Bearings",
            priority="low",
            message="Vibration slightly elevated",
            reason=f"Vibration: {state.vibration_mms:.2f} mm/s (normal: <3.5 mm/s)"
        ))
    
    # Operating hours checks - multiple milestones
    if state.operating_hours > 10000:
        recommendations.append(MaintenanceRecommendation(
            component="System",
            priority="high",
            message="Major service overdue",
            reason=f"Operating hours: {state.operating_hours:.0f}h (major service: 10000h)"
        ))
    elif state.operating_hours > 8000:
        recommendations.append(MaintenanceRecommendation(
            component="Gearbox",
            priority="medium",
    ...
    elif state.operating_hours > 7500:
        recommendations.append(MaintenanceRecommendation(
            component="System",
            priority="low",
            message="Approaching service interval",
            reason=f"Operating hours: {state.operating_hours:.0f}h (next service: 8000h)"
        ))
    elif state.operating_hours > 5000 and state.operating_hours % 1000 < 100:
        # Show milestone markers every 1000 hours after 5000h
        recommendations.append(MaintenanceRecommendation(
            component="System",
            priority="low",
            message=f"Operating hours milestone: {int(state.operating_hours/1000)}k hours",
            reason=f"Total runtime: {state.operating_hours:.0f}h"
        ))
    
    # Bearing temperature (added for more realistic monitoring)
    if state.bearing_temp_c > 85:
        recommendations.append(MaintenanceRecommendation(
            component="Bearings",
            priority="high",
            message="Bearing temperature excessive",
            reason=f"Temperature: {state.bearing_temp_c:.1f}°C (limit: 85°C)"
        ))
    elif state.bearing_temp_c > 75:
        recommendations.append(MaintenanceRecommendation(
            component="Bearings",
            priority="medium",
            message="Bearing temperature elevated",
            reason=f"Temperature: {state.bearing_temp_c:.1f}°C (warning: 75°C)"
        ))
...
async def get_state_history(
    ...
    """Get historical state data (v1.0 with robust error handling)."""
    try:
        sim = get_simulator()
        ...
        # v1.0: Safe status conversion helper
        def safe_status(status) -> str:
            """Convert status safely to string."""
            try:
                if isinstance(status, str):
                    return status
                elif isinstance(status, int):
                    status_map = {0: "STOPPED", 1: "STARTING", 2: "RUNNING", 
                                  3: "RATED_POWER", 4: "STOPPING", 5: "EMERGENCY_STOP",
                                  6: "FAULT", 7: "MAINTENANCE"}
                    return status_map.get(status, f"UNKNOWN_{status}")
                elif hasattr(status, 'name'):
                    return status.name
                elif hasattr(status, 'value'):
                    return str(status.value) if isinstance(status.value, str) else status.name if hasattr(status, 'name') else str(status.value)
                else:
                    return str(status)
            except Exception:
                return "UNKNOWN"
        ...
        data = [
            HistoryPoint(
                timestamp=state.timestamp,
                power_kw=max(0.0, min(10000.0, state.power_output_kw)),  # Clamp
                wind_speed_ms=max(0.0, min(50.0, state.wind_speed_ms)),  # Clamp
                status=safe_status(state.status)
            )
        ...
        print(f"❌ Error in get_state_history: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(
            status_code=503,
            detail="Temporary error accessing history - system is recovering"
        )
...
def initialize_simulator(days: int = 1, forecast: bool = True):
    """Initialize the global simulator instance (v1.0 enhanced with real-time timestamps)."""
    ...
    print("  ✓ Continuous weather ready (smooth transitions enabled)")
    print()
    
    # v1.0 FIX: Generate weather data starting from CURRENT TIME (not 2024-01-01)
    # This makes timestamps realistic and current
    now = datetime.now()
    start_date_str = now.strftime("%Y-%m-%d")
    
    print(f"Generating initial weather data starting from {start_date_str}...")
    weather_gen = WeatherGenerator(seed=42)
    weather_data = weather_gen.generate(
        start_date=start_date_str,  # Use current date
        days=days,
        interval_minutes=10
    )
    
    # v1.0 FIX: Adjust timestamps to match current wall-clock time
    # Replace DataFrame timestamps with current time + offsets
    time_offsets = [(i * 10 * 60) for i in range(len(weather_data))]  # 10-minute intervals in seconds
    current_timestamps = [now + timedelta(seconds=offset) for offset in time_offsets]
    weather_data['timestamp'] = current_timestamps
    
    print(f"  ✓ Generated {len(weather_data)} weather samples")
    print(f"  ✓ Time range: {weather_data['timestamp'].iloc[0].strftime('%Y-%m-%d %H:%M')} to {weather_data['timestamp'].iloc[-1].strftime('%Y-%m-%d %H:%M')}")
    print()
    ...
    if forecast:
        ...
        if model_path.exists():
            ...
            print("  ✓ Forecast model found")
        else:
            print("  ⚠ Forecast model not found, continuing without forecasts")
    
    print("Initializing simulator...")
    _simulator = Simulator(weather_data, forecaster_path=forecaster_path)
    print(f"  ✓ Turbine: {_simulator.config.turbine.name}")
    print(f"  ✓ Rated Power: {_simulator.config.turbine.rated_power_kw} kW")
    if _simulator.forecast_enabled:
        print("  ✓ Forecast: Enabled")
    
    # v1.0: Run initial step to populate current_state
    print()
    print("Running initial simulation step...")
    _simulator.step()
    initial_state = _simulator.state_manager.current_state
    if initial_state:
        print(f"  ✓ Initial state: {initial_state.timestamp.strftime('%Y-%m-%d %H:%M:%S')} | "
              f"Power: {initial_state.power_output_kw:.0f} kW | "
              f"Status: {safe_status_string(initial_state.status)}")
    print()
...
if __name__ == "__main__":
    ...
    if static_path.exists():
        ...
        print(f"✓ Serving web UI from {static_path}")
        print()
